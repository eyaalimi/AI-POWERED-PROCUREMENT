...
- For EVERY supplier you evaluate (retained or excluded), call log_sourcing_decision() with:
  ...
  - reason: a clear explanation (e.g. "relevant to IT Equipment, professional .tn website", "marketplace site not a real supplier", "no contact email found after scraping")
- This creates an audit trail so we know WHY each supplier was retained or excluded.
...
class SourcingAgent:
    ...
    def __init__(self):
        ...
        self._agent = Agent(
            ...
            tools=[
                search_existing_suppliers,
                search_suppliers,
                get_supplier_contact,
                log_sourcing_decision,
            ],
        )
