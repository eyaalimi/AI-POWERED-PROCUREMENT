...
from db.models import (
    ProcurementRequest, Supplier, RFQ, Offer, Evaluation, PipelineEvent, User,
)
...
def get_pipelines(
    ...
    if current_user.role == "employee":
        query = query.filter(ProcurementRequest.created_by_id == current_user.id)
    if status:
    ...
    for req in requests:
        ...
        best_eval = db.query(Evaluation).filter_by(request_id=req.id, rank=1).first()
        ...
        results.append({
            ...
            "requester_email": req.requester_email, "status": req.status,
            "created_at": req.created_at.isoformat() if req.created_at else None,
            "budget_max": req.budget_max, "deadline": req.deadline,
            "stages": stages, "suppliers_found": supplier_count,
            "rfqs_sent": rfq_count, "offers_received": offer_count,
...
def get_pipeline_detail(request_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    rfqs = [
        {"supplier_id": str(r.supplier_id), "subject": r.subject, "status": r.status,
         "sent_at": r.sent_at.isoformat() if r.sent_at else None, "reminder_sent": r.reminder_sent}
        for r in db.query(RFQ).filter_by(request_id=req_uuid).all()
    ]
    offers = [
        {"supplier_id": str(o.supplier_id), "unit_price": o.unit_price, "total_price": o.total_price,
    ...
    events = [
        {"agent": ev.agent, "event_type": ev.event_type, "message": ev.message,
    ...
    return {
        ...
        "suppliers": suppliers, "rfqs": rfqs, "offers": offers,
        "evaluations": evaluations, "events": events,
    }
